...
{
	hmi.Es.clear();
	if (hmi.Hs.size() && hmi.type != Mesh_type::Hyb)
		hmi.Fs.clear();
	// either hex or tri
	...
	else if (hmi.type == Mesh_type::Tet)
	{
		...
		for (uint32_t i = 0; i < hmi.Hs.size(); ++i)
		{
			...
			{
				for (short k = 0; k < 3; k++)
					vs[k] = hmi.Hs[i].vs[tet_faces[j][k]];
				uint32_t id = 4 * i + j;
				total_fs[id] = vs;
	...
	else if (hmi.type == Mesh_type::Hex)
	{
		...
		for (uint32_t i = 0; i < hmi.Hs.size(); ++i)
		{
			...
			{
				for (short k = 0; k < 4; k++)
					vs[k] = hmi.Hs[i].vs[hex_face_table[j][k]];
				uint32_t id = 6 * i + j;
				total_fs[id] = vs;
				std::sort(vs.begin(), vs.end());
	...
	else if (hmi.type == Mesh_type::Hyb)
	{
		...
		for (auto h : hmi.Hs)
			for (auto fid : h.fs)
				hmi.Fs[fid].neighbor_hs.push_back(h.id);
		for (auto &f : hmi.Fs)
		...
		for (auto f : hmi.Fs)
			if (f.boundary)
				for (uint32_t j = 0; j < f.es.size(); ++j)
				{
					uint32_t eid = f.es[j];
					hmi.Es[eid].boundary = true;
					hmi.Vs[hmi.Es[eid].vs[0]].boundary = hmi.Vs[hmi.Es[eid].vs[1]].boundary = true;
	...
	for (uint32_t i = 0; i < hmi.Hs.size(); i++)
	{
		for (uint32_t j = 0; j < hmi.Hs[i].fs.size(); j++)
			hmi.Fs[hmi.Hs[i].fs[j]].neighbor_hs.push_back(i);
	}
	// e_nfs, v_nfs
	for (auto &e : hmi.Es)
		e.neighbor_fs.clear();
	...
	for (uint32_t i = 0; i < hmi.Fs.size(); i++)
	{
		for (uint32_t j = 0; j < hmi.Fs[i].es.size(); j++)
			hmi.Es[hmi.Fs[i].es[j]].neighbor_fs.push_back(i);
		for (uint32_t j = 0; j < hmi.Fs[i].vs.size(); j++)
			hmi.Vs[hmi.Fs[i].vs[j]].neighbor_fs.push_back(i);
	}
	// v_nes, v_nvs
	for (auto &v : hmi.Vs)
	{
	...
	for (uint32_t i = 0; i < hmi.Hs.size(); i++)
	{
		for (uint32_t j = 0; j < hmi.Hs[i].vs.size(); j++)
			hmi.Vs[hmi.Hs[i].vs[j]].neighbor_hs.push_back(i);
	}
}
void topology_info(Mesh &mesh, Frame &frame, Mesh_Topology &mt)
{
	...
	if (mt.surface_euler != 2 * mt.volume_euler)
	{
		cout << "Surface euler not equal to volume euler" << endl;
		mt.euler_problem = true;
	}
	// surface_manifoldness;
	mt.manifoldness_problem = false;
	mt.surface_manifoldness = true;
	vector<short> E_flag(mesh.Es.size(), 0);
	vector<short> V_flag(mesh.Vs.size(), 0);
	...
	//==================frame==================//
	for (int i = 0; i < frame.FVs.size(); i++)
		if (!frame.FVs[i].neighbor_fhs.size())
			cout << "Frame Topology ERROR double check!" << endl;
	// surface_euler, volume_euler;
	num_boundary_v = num_boundary_e = num_boundary_f = 0;
	for (int i = 0; i < frame.FVs.size(); i++)
		if (frame.FVs[i].boundary)
			num_boundary_v++;
	for (int i = 0; i < frame.FEs.size(); i++)
		if (frame.FEs[i].boundary)
			num_boundary_e++;
	for (int i = 0; i < frame.FFs.size(); i++)
		if (frame.FFs[i].boundary)
			num_boundary_f++;
	...
	if (mt.frame_surface_euler != 2 * mt.frame_volume_euler)
	{
		cout << "Frame surface euler not equal to volume euler" << endl;
		mt.frame_euler_problem = true;
	}
	// surface_manifoldness;
	mt.frame_manifoldness_problem = false;
	mt.frame_surface_manifoldness = true;
	E_flag.resize(frame.FEs.size());
	V_flag.resize(frame.FVs.size());
	for (uint32_t i = 0; i < frame.FVs.size(); i++)
	{ // topology disk
		if (!frame.FVs[i].boundary)
			continue;
		vector<vector<uint32_t>> fes;
		...
		{
			if (frame.FFs[fid].boundary)
				fes.push_back(frame.FFs[fid].es);
		}
	...
	{ // topology sphere
		...
		for (auto hid : frame.FVs[i].neighbor_fhs)
			pfs.push_back(frame.FHs[hid].fs);
...
bool comp_topology(Mesh_Topology &mt0, Mesh_Topology &mt1)
{
	...
	if (mt1.manifoldness_problem)
		return false;
	if (mt0.genus != mt1.genus)
		return false;
	if (mt0.frame_genus != mt1.frame_genus)
		return false;
	return true;
}
bool redundentV_check(Mesh &meshI, Mesh &meshO)
{
	...
	for (auto v : meshI.Vs)
	{
		if (!v.neighbor_hs.size() || !v.neighbor_fs.size())
		{
			redundentV = true;
			continue;
		}
	...
	meshO.Hs = meshI.Hs;
	Hybrid h;
	if (meshO.type == Mesh_type::Hex)
		h.vs.resize(8);
	else if (meshO.type == Mesh_type::Tet)
		h.vs.resize(4);

	for (auto &h : meshO.Hs)
	{
		for (uint32_t j = 0; j < h.vs.size(); j++)
			h.vs[j] = V_tag[h.vs[j]];
		for (uint32_t i = 0; i < h.vs.size(); i++)
			meshO.Vs[h.vs[i]].neighbor_hs.push_back(h.id);
	}
	return redundentV;
}
...
double global_boundary_ratio(Mesh &mesh, bool medial_surface)
{
	...
	for (const auto &v : mesh.Vs)
	{
		if (medial_surface && v.on_medial_surface)
			N++;
		else if (!medial_surface && v.boundary)
			N++;
	}
	double ratio = 0;
	if (N > 0)
		ratio = mesh.Vs.size() / double(N);
...
void re_indexing_connectivity(Mesh &hmi, MatrixXi &H)
{
	...
	for (uint32_t i = 0; i < H.cols(); i++)
	{
		...
		hmi.Hs[h.id] = h;
		for (uint32_t i = 0; i < h.vs.size(); i++)
			hmi.Vs[h.vs[i]].neighbor_hs.push_back(h.id);
	}
...
void re_indexing_connectivity_sur(Mesh &mi, vector<bool> &f_flag, Mesh &mo, vector<int32_t> &V_map, vector<int32_t> &V_map_reverse, vector<int32_t> &f_map, vector<int32_t> &f_map_reverse)
{
	...
	for (uint32_t j = 0; j < f_flag.size(); j++)
		if (f_flag[j])
		{
			for (auto fvid : mi.Fs[j].vs)
				v_tag_[fvid] = true;
		}
	V_map.resize(v_tag_.size(), -1);
	for (uint32_t j = 0; j < v_tag_.size(); j++)
...
void triangulation(Mesh &meshi, Mesh &mesho)
{
	...
	for (const auto &f : meshi.Fs)
		for (int i = 2; i < f.vs.size(); i++)
		{
...
void extract_surface_mesh(Mesh &meshi, Mesh &mesho)
{
	...
	for (uint32_t i = 0; i < mesho.Fs.size(); i++)
		for (uint32_t j = 0; j < 3; j++)
			mesho.Fs[i].vs[j] = V_map[mesho.Fs[i].vs[j]];
	// orient direction
	build_connectivity(mesho);
	orient_surface_mesh(mesho);
}
void extract_surface_conforming_mesh(Mesh &meshi, Mesh &mesho, vector<int32_t> &V_map, vector<int32_t> &V_map_reverse, vector<int32_t> &F_map, vector<int32_t> &F_map_reverse)
{
	...
	vector<bool> V_tag(meshi.Vs.size(), false);
	F_map.resize(meshi.Fs.size());
	fill(F_map.begin(), F_map.end(), -1);
	V_map.resize(meshi.Vs.size());
	fill(V_map.begin(), V_map.end(), -1);
	V_map_reverse.clear();
	F_map_reverse.clear();
	for (auto f : meshi.Fs)
		if (f.boundary)
...
void orient_surface_mesh(Mesh &hmi)
{
	...
	while (!pf_temp.empty())
	{
		...
		for (auto eid : hmi.Fs[fid].es)
			for (auto nfid : hmi.Es[eid].neighbor_fs)
			{
				if (!flag[nfid])
					continue;
				...
				if ((v0_pos + 1) % hmi.Fs[fid].vs.size() != v1_pos)
					swap(v0, v1);
				...
				if ((v0_pos_ + 1) % hmi.Fs[nfid].vs.size() == v1_pos_)
					std::reverse(hmi.Fs[nfid].vs.begin(), hmi.Fs[nfid].vs.end());
	...
	{
		for (uint32_t i = 0; i < hmi.Fs.size(); i++)
			std::reverse(hmi.Fs[i].vs.begin(), hmi.Fs[i].vs.end());
	}
}
...
{
	vector<vector<Float>> p(hmi.V.cols());
	vector<vector<uint32_t>> f(hmi.Fs.size());
	for (uint32_t i = 0; i < hmi.V.cols(); i++)
	{
		vector<Float> pv;
...
void orient_triangle_mesh(MatrixXd &Tri_V, MatrixXi &Tri_F)
{
	...
	vector<vector<Float>> p(Tri_V.cols());
	vector<vector<uint32_t>> f(Tri_F.rows());
...
void face_soup_roll_region(Mesh &hmi, uint32_t sfid, vector<bool> &E_tag, vector<uint32_t> &bes, vector<uint32_t> &fs, vector<uint32_t> &bvs)
{
	...
	for (auto eid : hmi.Fs[sfid].es)
	{
		if (E_tag[eid])
		{
			svid = hmi.Es[eid].vs[0];
			bvs.push_back(hmi.Es[eid].vs[0]);
			cvid = hmi.Es[eid].vs[1];
			bes.push_back(eid);
			pre_eid = eid;
			break;
		}
	...
	while (cvid != svid)
	{
		...
		do
		{
			...
			for (auto neid : hmi.Vs[cvid].neighbor_es)
				if (neid != pre_eid && find(es.begin(), es.end(), neid) != es.end())
				{
					eid = neid;
					break;
				}
			...
			for (auto nfid : hmi.Es[eid].neighbor_fs)
				if (hmi.Fs[nfid].boundary && nfid != sfid)
				{
					sfid = nfid;
					break;
				}
		...
		bes.push_back(eid);
		if (hmi.Es[eid].vs[0] == cvid)
			cvid = hmi.Es[eid].vs[1];
		else
			cvid = hmi.Es[eid].vs[0];
	}
	for (auto eid : bes_)
		E_tag[eid] = false;
}
	...
	while (true)
	{
		...
		for (auto fid : fs_soup_)
		{
			for (auto eid : hmi.Fs[fid].es)
				...
				{
					for (auto nfid : hmi.Es[eid].neighbor_fs)
						if (hmi.Fs[nfid].boundary && !F_tag[nfid])
						{
		...
		while (true)
		{
			...
			for (auto hid : group_)
			{
				...
				for (uint32_t i = 0; i < 6; i++)
					for (uint32_t j = 0; j < 4; j++)
						Fvs[i].push_back(hmi.Hs[hid].vs[hex_face_table[i][j]]);
				fvs_sorted = Fvs;
				for (auto &vs : fvs_sorted)
					sort(vs.begin(), vs.end());

				for (auto fid : hmi.Hs[hid].fs)
					if (!hmi.Fs[fid].boundary)
					{
						int nhid = hmi.Fs[fid].neighbor_hs[0];
						if (nhid == hid)
							nhid = hmi.Fs[fid].neighbor_hs[1];
						...
						if (!H_tag[nhid])
						{
							...
							{
								for (auto nvid : hmi.Vs[topvs[i]].neighbor_vs)
									if (nvid != topvs[(i + 3) % 4] && nvid != topvs[(i + 1) % 4] && std::find(hvs.begin(), hvs.end(), nvid) != hvs.end())
									{
...
bool scaled_jacobian(Mesh &hmi, Mesh_Quality &mq)
{
	if (hmi.type == Mesh_type::Hex)
	{
		...
		mq.V_Js.resize(hmi.Hs.size() * 8);
		mq.V_Js.setZero();
		mq.H_Js.resize(hmi.Hs.size());
		mq.H_Js.setZero();
	...
	else if (hmi.type == Mesh_type::Qua)
	{ // 2D planar only!
		mq.ave_Jacobian = 0;
		mq.min_Jacobian = 1;
		mq.deviation_Jacobian = 0;
		mq.V_Js.resize(hmi.Fs.size() * 4);
		mq.V_Js.setZero();
		mq.H_Js.resize(hmi.Fs.size());
		mq.H_Js.setZero();
	...
	else if (hmi.type == Mesh_type::Tet)
	{
		...
		for (uint32_t i = 0; i < hmi.Hs.size(); i++)
		{
			...
			if (mq.min_Jacobian > jacobian_value)
				mq.min_Jacobian = jacobian_value;
		}
		mq.ave_Jacobian /= hmi.Hs.size();
		for (int i = 0; i < mq.H_Js.size(); i++)
			mq.deviation_Jacobian += (mq.H_Js[i] - mq.ave_Jacobian) * (mq.H_Js[i] - mq.ave_Jacobian);
...
double a_jacobian(VectorXd &v0, VectorXd &v1, VectorXd &v2, VectorXd &v3)
{
	...
	if (std::abs(norm1) < Precision || std::abs(norm2) < Precision || std::abs(norm3) < Precision)
	{
		...
		std::cout << "Potential Bug, check!" << endl;
		system("PAUSE");
	}
	scaled_jacobian /= norm1 * norm2 * norm3;
	return scaled_jacobian;
...
bool quad_mesh_feature(Mesh_Feature &mf)
{
	...
	// normals
	mf.normal_Tri.resize(3, mesh.Fs.size());
	mf.normal_Tri.setZero();
	mf.normal_V.resize(3, mesh.Vs.size());
	mf.normal_V.setZero();
	mf.Tcenters.clear();
	mf.Tcenters.resize(mesh.Fs.size());
	for (uint32_t i = 0; i < mesh.Fs.size(); i++)
	{
	...
	for (uint32_t i = 0; i < mf.normal_V.cols(); ++i)
		if (mf.normal_V.col(i) != Vector3d::Zero())
			mf.normal_V.col(i) = mf.normal_V.col(i).normalized();
	mf.ave_length = 0;
	for (uint32_t i = 0; i < mesh.Es.size(); i++)
	{
...
bool triangle_mesh_feature(Mesh_Feature &mf)
{
	...
	mf.v_types.resize(mesh.Vs.size());
	std::fill(mf.v_types.begin(), mf.v_types.end(), 0);
	if (mf.read_from_file)
	{
		for (auto &v : mesh.Vs)
			sort(v.neighbor_es.begin(), v.neighbor_es.end());
		...
		for (uint32_t i = 0; i < mf.v_types.size(); i++)
			if (mf.v_types[i] == 0)
				mf.v_types[i] = Feature_V_Type::REGULAR;
	...
	for (uint32_t i = 0; i < mesh.Es.size(); i++)
	{
		...
		{
			if (mf.v_types[vid] == Feature_V_Type::REGULAR || mf.v_types[vid] == Feature_V_Type::CORNER)
				return false;

			for (uint32_t j = 0; j < mesh.Vs[vid].neighbor_es.size(); j++)
			{
				uint32_t cur_e = mesh.Vs[vid].neighbor_es[j];
				if (cur_e == eid || !E_feature_flag[cur_e] || E_flag[cur_e])
					continue;
				neid = cur_e;
				return true;
			}
		...
		while (feature_line_proceed(v_left, cur_e, next_e))
		{
			...
			if (mesh.Es[cur_e].vs[0] == v_left)
				v_left = mesh.Es[cur_e].vs[1];
			else
				v_left = mesh.Es[cur_e].vs[0];
			vs_left.push_back(v_left);
		}
		...
		if (is_circle)
		{
			...
			mf.curve_es.push_back(es_left);
			mf.curve_vs.push_back(vs_left);
			continue;
		}
		// right
		...
		while (feature_line_proceed(v_right, cur_e, next_e))
		{
			cur_e = next_e;
			E_flag[next_e] = true;
			if (mesh.Es[cur_e].vs[0] == v_right)
				v_right = mesh.Es[cur_e].vs[1];
			else
				v_right = mesh.Es[cur_e].vs[0];
			vs_right.push_back(v_right);
		...
		vs_link = vs_left;
		vs_link.insert(vs_link.end(), vs_right.begin(), vs_right.end());
		std::reverse(es_left.begin(), es_left.end());
		vector<uint32_t> es_link;
		es_link = es_left;
		es_link.insert(es_link.end(), es_right.begin(), es_right.end());

		if (vs_link[0] == vs_link[vs_link.size() - 1])
		{ // one corner with a circle
			vector<uint32_t> vs_ = vs_link;
			vs_link.clear();
			for (uint32_t j = 0; j < vs_.size() - 1; j++)
				vs_link.push_back(vs_[j]);
			...
			for (int j = 0; j < curve_vs1.size() - 1; j++)
			{
				...
				set_intersection(es0.begin(), es0.end(), es1.begin(), es1.end(), back_inserter(sharedes));
				if (sharedes.size())
					es_link1.push_back(sharedes[0]);
				else
				{
					cout << "bug" << endl;
				}
			}
			mf.curve_es.push_back(es_link0);
			mf.curve_vs.push_back(curve_vs0);
			mf.circles.push_back(false);
			mf.curve_es.push_back(es_link1);
			mf.curve_vs.push_back(curve_vs1);
			mf.circles.push_back(false);
		}
		else
		{
			mf.curve_es.push_back(es_link);
			mf.curve_vs.push_back(vs_link);
			mf.circles.push_back(false);
		}
	}
	mf.corner_curves.clear();
	mf.corner_curves.resize(mf.corners.size());
	//
	for (uint32_t i = 0; i < mf.curve_vs.size(); i++)
		for (auto vid : mf.curve_vs[i])
			if (mf.v_types[vid] != Feature_V_Type::CORNER)
				mf.v_types[vid] = i;
			else
			{
	...
	if (!mf.orphan_curve || !mf.orphan_curve_single)
	{
		while (true)
		{
			...
			for (uint32_t i = 0; i < mf.corner_curves.size(); i++)
				if (mf.corner_curves[i].size() == 1)
				{
			...
			for (uint32_t i = 0; i < mf.corner_curves.size(); i++)
				if (mf.corner_curves[i].size() == 1)
				{
					if (l_flag[mf.corner_curves[i][0]])
						c_flag[i] = true;
				}
				else if (!mf.corner_curves[i].size())
					c_flag[i] = true;

			vector<uint32_t> corner_;
			vector<bool> circles_;
			...
			for (uint32_t i = 0; i < mf.corners.size(); i++)
			{
				...
				vector<uint32_t> cc;
				for (auto lid : mf.corner_curves[i])
					if (L_map[lid] != -1)
						cc.push_back(L_map[lid]);
		...
		for (auto l : mf.curve_vs)
			for (auto vid : l)
				mf.v_types[vid] = Feature_V_Type::LINE;
		for (auto c : mf.corners)
			mf.v_types[c] = Feature_V_Type::CORNER;
	}
...
void build_feature_graph(Mesh_Feature &mf, Feature_Graph &fg)
{
	...
	for (uint32_t i = 0; i < fg.Cs.size(); i++)
	{
		...
		for (auto cid : mf.corner_curves[i])
		{
			...
			if (curve[0] == mf.corners[i])
				vs.push_back(curve[1]);
			else if (curve[curve.size() - 1] == mf.corners[i])
	...
	while (true)
	{
		...
		for (uint32_t i = 0; i < f_flag.size(); i++)
			if (f_flag[i] == INVALID_F)
			{
				start_f = i;
				break;
			}
		if (start_f == INVALID_F)
			break;

		std::queue<uint32_t> f_pool;
		f_pool.push(start_f);
		while (!f_pool.empty())
		{
			start_f = f_pool.front();
			f_pool.pop();
			if (f_flag[start_f] != INVALID_F)
				continue;
	...
	{
		for (auto lid : fg.Rs[i].ls)
			fg.Ls[lid].neighbor_rs.push_back(i);
		for (auto lid : fg.Rs[i].ls)
			for (auto cid : fg.Ls[lid].cs)
				fg.Rs[i].cs.push_back(cid);
...
bool triangle_mesh_feature(Mesh_Feature &mf, Mesh &hmi)
{
	...
	mf.V_map_reverse.clear();
	mf.V_map.resize(hmi.Vs.size());
	std::fill(mf.V_map.begin(), mf.V_map.end(), -1);
	...
	for (uint32_t i = 0; i < mesh.Fs.size(); i++)
		for (uint32_t j = 0; j < 3; j++)
			mesh.Fs[i].vs[j] = mf.V_map[mesh.Fs[i].vs[j]];
	// orient direction
	...
	vector<bool> E_feature_flag(mesh.Es.size(), false);
	mf.v_types.resize(mesh.Vs.size());
	fill(mf.v_types.begin(), mf.v_types.end(), 0);
	vector<Float> Dihedral_angles(mesh.Es.size());
	for (uint32_t i = 0; i < mesh.Es.size(); i++)
	{
		Vector3d n0 = mf.normal_Tri.col(mesh.Es[i].neighbor_fs[0]);
...
bool project_surface_update_feature(Mesh_Feature &mf, Feature_Constraints &fc, MatrixXd &V, VectorXi &b, MatrixXd &bc, uint32_t Loop, uint32_t grain_size)
{
		...
		[&](const tbb::blocked_range<uint32_t> &range)
		{
			for (uint32_t m = range.begin(); m != range.end(); m++)
			{
				...
				else if (type == Feature_V_Type::REGULAR)
				{
					...
					tid = fc.V_ids[i];
					if (!fc.RV_type[i])
						tids = mf.tri.Vs[tid].neighbor_fs;
					else
						tids.push_back(tid);

					Vector3d PreinterpolP = fc.V_T.row(mi), PreinterpolN = fc.normal_T.row(mi);
					bool found = phong_projection(mf, tids, Loop, tid, v, interpolP, interpolN, PreinterpolP, PreinterpolN);
					...
					if (!found || (v - interpolP).norm() >= mf.ave_length)
					{
						...
						if (phong_projection(mf, tids, Loop, tid_temp, v, interpolP, interpolN, PreinterpolP, PreinterpolN))
							tid = tid_temp;
						else
						{
...
bool phong_projection(Mesh_Feature& mf, vector<uint32_t> &tids, uint32_t Loop, uint32_t &tid, Vector3d &v, Vector3d &interpolP, Vector3d &interpolN, Vector3d &PreinterpolP, Vector3d &PreinterpolN)
{
	...
	tids.erase(unique(tids.begin(), tids.end()), tids.end());
	...
	while (ts.size())
	{
		...
		for (uint32_t j = 0; j < ts.size(); j++)
		{
				...
				(1 - uv.x() - uv.y() >= 0.0 || num_equal(1 - uv.x() - uv.y(), 0.0, Precision_Pro)))
			{
				...
				if (PreinterpolN != Vector3d::Zero() && interpolN.dot(PreinterpolN) <= 0)
					continue;

				dis_ids.push_back(make_pair((v - interpolP).norm(), tids.size()));
				pvs.push_back(interpolP);
				pns.push_back(interpolN);
				uvs.push_back(uv);
				tids.push_back(ts[j]);
			}
		}
...
void nearest_point_loop(const MatrixXd &P, const MatrixXd &V, VectorXi &I)
{
	...
	{
		double max_dis = (P.row(i) - V.row(0)).norm();
		int id = 0;
...
void hausdorff_dis(Mesh &mesh0, Mesh &mesh1)
{
	...
	for (uint32_t i = 0; i < mesh0.Fs.size(); i++)
		for (uint32_t j = 0; j < 3; j++)
			FA(i, j) = mesh0.Fs[i].vs[j];
	for (uint32_t i = 0; i < mesh1.Fs.size(); i++)
		for (uint32_t j = 0; j < 3; j++)
			FB(i, j) = mesh1.Fs[i].vs[j];
	double dis = 0;
...
bool hausdorff_dis(Mesh &mesh0, Mesh &mesh1, double &hausdorff_ratio_threshould)
{
	...
	if (ratio < hausdorff_ratio_threshould)
		return true;
	return false;
}
bool hausdorff_dis(Mesh &mesh0, Mesh &mesh1, vector<int> &outlierVs, double &hausdorff_dis_threshold)
{
	...
	while (!outlierVs.size())
	{
		...
		{
			if (sqr_DBA[i] > threshold)
				if (!flag[i])
				{
					outlierVs.push_back(i);
					flag[i] = true;
				}
		}
