...
void grid_hex_meshing_bijective::pipeline(string path)
{
	...
	while (true)
	{
		...
		for (uint32_t j = 0; j < local_refinement_ringN; j++)
		{
			...
			sort(nvs_.begin(), nvs_.end());
			nvs_.erase(unique(nvs_.begin(), nvs_.end()), nvs_.end());
		...
		if (STOP_EXTENT_MIN < 1)
			break;
		if (STOP_EXTENT_MAX < 1)
			STOP_EXTENT_MAX = 1;
...
bool grid_hex_meshing_bijective::pure_hex_meshing()
{
	...
	if (!MD.mesh_entire.Hs.size())
	{
		cout << "no hex-mesh generated, exit!";
		return false;
	}
	...
	{
		cout << "no hex-mesh inside the object, exit!";
		return false;
	}
...
void grid_hex_meshing_bijective::dual_octree_meshing(GEO::Mesh &mi, Mesh &mo)
{
	...
	for (uint32_t i = 0; i < h_type.size(); i++)
		if (h_type[i] == Element_Type::Pyramid)
		{
			for (uint32_t j = 0; j < 4; j++)
				vertex_type[hybrid_standard.Hs[i].vs[j]] = Vertex_Tag::Pyramid_bottom;
	...
	for (auto &f : hybrid_standard.Fs)
	{
		bool notip = false;
		for (auto vid : f.vs)
			if (vertex_type[vid] != Vertex_Tag::Pyramid_tip)
				notip = true;
		if (!notip)
			tip_quads[f.id] = true;
	}
	...
	for (uint32_t i = 0; i < tip_quads.size(); i++)
	{
		...
		{
			vector<int> nBV;
			for (auto hvid : hybrid_standard.Hs[nhid].vs)
				if (vertex_type[hvid] == Vertex_Tag::Pyramid_bottom)
					nBV.push_back(hvid);
			if (nBV.size() == 4)
			{
				int nP = 0;
				for (auto fid : hybrid_standard.Hs[nhid].fs)
					for (auto nnhid : hybrid_standard.Fs[fid].neighbor_hs)
						if (nnhid != nhid && h_type[nnhid] == Element_Type::Prism)
							nP++;
				if (nP != 4)
					continue;

				bc.hex_id = nhid;
				for (auto vid : bc.top_qvs)
					for (auto vid1 : nBV)
					{
		...
		for (auto eid : hybrid_standard.Fs[i].es)
		{
			...
			sort(fs_local.begin(), fs_local.end());
			sort(fs_hex.begin(), fs_hex.end());
			set_intersection(fs_local.begin(), fs_local.end(), fs_hex.begin(), fs_hex.end(), back_inserter(fs_shared));
			int side_fid = -1;
			for (auto fshared : fs_shared)
				if (fshared != i)
					side_fid = fshared;
			for (auto nhid : hybrid_standard.Fs[side_fid].neighbor_hs)
				if (nhid != bc.hex_id)
					bc.prism_ids.push_back(nhid);
		}
		// pyramid
		for (uint32_t j = 0; j < bc.prism_ids.size(); j++)
		{
			...
			vector<int> ps;
			for (auto pfid : hybrid_standard.Hs[prism].fs)
				for (auto nhid : hybrid_standard.Fs[pfid].neighbor_hs)
					if (h_type[nhid] == Element_Type::Pyramid)
						ps.push_back(nhid);
			for (auto p : ps)
				if (hybrid_standard.Hs[p].vs[4] == bc.top_qvs[j])
				{
					bc.pyramid_ids.push_back(p);
					break;
				}
		}
		// Vertices
		...
		for (uint32_t j = 0; j < 4; j++)
		{
			...
			for (uint32_t k = 0; k < 4; k++)
				if (bc.base_qvs[j] == hybrid_standard.Hs[pid].vs[k])
					which_index = k;
		...
		for (uint32_t j = 0; j < 4; j++)
		{
			...
			for (auto vid : hybrid_standard.Hs[pid].vs)
				if (vid == othervs[j][0] || vid == othervs[j][1])
					bc.base_bvs[j].push_back(vid);
			for (auto vid : hybrid_standard.Hs[pid].vs)
				if (vid == othervs[(j + 1) % 4][0] || vid == othervs[(j + 1) % 4][1])
					bc.base_bvs[j].push_back(vid);
		}
	...
	while (true)
	{
		...
		a_layer.push_back(f_start);
		layer_temp.push_back(f_start);
		while (layer_temp.size())
		{
			for (auto fid : layer_temp)
				for (auto vid : hybrid_standard.Fs[fid].vs)
					for (auto nfid : hybrid_standard.Vs[vid].neighbor_fs)
					{
						if (tq_tag[nfid] && tip_quads[nfid])
						{
							tq_tag[nfid] = false;
							layer_temp2.push_back(nfid);
						}
					}
			if (layer_temp2.size())
			{
											 ...
											 Eigen::Vector3i &grid_size)
{
	...
	auto should_subdivide = [&](int x, int y, int z, int extent)
	{
		...
		auto action = [&has_triangles](int id)
		{ has_triangles = true; };
	...
	{
		for (auto &tbv : tb_subdivided_cells)
			tbv = hex2Octree_map[tbv];
	...
	int numLeaves = 0;
	for (int c = 0; c < octree.numCells(); ++c)
		if (octree.cellIsLeaf(c))
		{
			++numLeaves;
		}
	mo.Hs.clear();
	mo.Hs.resize(numLeaves);
	for (int q = 0, c = 0; q < octree.numCells(); ++q)
	{
		if (!octree.cellIsLeaf(q))
			continue;
...
void grid_hex_meshing_bijective::conforming_mesh(Mesh &mo, Mesh &hybrid, OctreeGrid &octree, Eigen::Vector3i &grid_size)
{
	...
	for (uint32_t i = 0; i < octree.numNodes(); i++)
	{
		...
		if ((pos[0] == 0 || pos[0] == grid_size[0]) || (pos[1] == 0 || pos[1] == grid_size[1]) || (pos[2] == 0 || pos[2] == grid_size[2]))
			v_boundary[i] = true;
	}
	...
	for (uint32_t i = 0; i < mo.Vs.size(); i++)
	{
		if (mo.Vs[i].boundary && !v_boundary[i])
		{
			...
			if (find(n.neighNodeId.begin(), n.neighNodeId.end(), -1) != n.neighNodeId.end())
			{ // T-node
				...
				if (pos0 % 2 == 1)
					pos1 = pos0 - 1;
				vector<int> vs; // 4 vs
				for (uint32_t j = 0; j < n.neighNodeId.size(); j++)
					if (j == pos0 || j == pos1)
						continue;
					else
						vs.push_back(n.neighNodeId[j]);
				if (find(vs.begin(), vs.end(), -1) != vs.end())
					continue; // notT-node  but at two sides
							  // 4 faces
				...
				sort(vs_total.begin(), vs_total.end());
				vs_total.erase(std::unique(vs_total.begin(), vs_total.end()), vs_total.end());
				// 4 corners
				vs.push_back(i);
				vector<int> vs_corners;
				for (auto vid : vs_total)
					if (find(vs.begin(), vs.end(), vid) != vs.end())
						continue;
					else
						vs_corners.push_back(vid);
				// big face
				sort(vs_corners.begin(), vs_corners.end());
				if (vs_corners.size() == 0)
				{
					cout << "bug check" << endl;
					system("PAUSE");
				}
	...
	for (uint32_t i = 0; i < f_relations.size(); i++)
	{
		if (f_relations[i].size() == 4)
		{
			...
			for (auto eid : mo.Fs[i].es)
			{
				...
				sort(nvs0.begin(), nvs0.end());
				sort(nvs1.begin(), nvs1.end());
				vector<uint32_t> vs_shared;
	...
	for (uint32_t i = 0; i < mo.Hs.size(); i++)
	{
		...
		for (auto vid : h.vs)
			hybrid.Vs[vid].neighbor_hs.push_back(h.id);
	}
...
void grid_hex_meshing_bijective::dual_conforming_mesh(Mesh &mo, Mesh &hybrid, Mesh &hybrid_standard, vector<Element_Type> &h_type)
{
	...
	for (uint32_t i = 0; i < hybrid.Es.size(); i++)
	{
		...
		for (int j = 0; j < hs.size(); j++)
		{
			...
			{
				if (hybrid.Fs[fid].neighbor_hs.size() == 1)
					continue;

				uint32_t hid = hybrid.Fs[fid].neighbor_hs[0];
				if (hid == sh)
					hid = hybrid.Fs[fid].neighbor_hs[1];
	...
	{
		if (hybrid.Vs[i].boundary)
			continue;
		Hybrid h;
		h.id = hybrid_standard.Hs.size();
		for (auto eid : hybrid.Vs[i].neighbor_es)
			h.fs.push_back(e_tag[eid]);
		...
		for (auto vid : h.vs)
			hybrid_standard.Vs[vid].neighbor_hs.push_back(h.id);
	}
	...
	for (auto &h : hybrid_standard.Hs)
	{
		...
		for (auto fid : h.fs)
		{
			if (hybrid_standard.Fs[fid].vs.size() == 3)
			{
				triN++;
				tris.push_back(fid);
			}
			else if (hybrid_standard.Fs[fid].vs.size() == 4)
			{
				quadN++;
				quads.push_back(fid);
			}
		...
		if (h.fs.size() == 4)
		{
			...
			else if (triN == 2 && quadN == 2)
			{
				...
				if (find(vs2.begin(), vs2.end(), v0) != vs2.end())
				{
					...
					for (uint32_t j = 0; j < vs2.size(); j++)
						h.vs.push_back(vs2[(id + j) % vs2.size()]);
				}
				else
				{
					id = find(vs3.begin(), vs3.end(), v0) - vs3.begin();
					for (uint32_t j = 0; j < vs3.size(); j++)
						h.vs.push_back(vs3[(id + j) % vs3.size()]);
				}
		...
		else if (h.fs.size() == 5)
		{
			if (triN == 4 && quadN == 1)
			{
				...
				for (auto vid : hybrid_standard.Fs[tris[0]].vs)
					if (find(h.vs.begin(), h.vs.end(), vid) != h.vs.end())
						continue;
					else
					{
						h.vs.push_back(vid);
			...
			else if (triN == 2 && quadN == 3)
			{
				...
				for (auto vid : hybrid_standard.Fs[tris[0]].vs)
					for (auto vid2 : hybrid_standard.Fs[tris[1]].vs)
					{
				...
				for (auto fid : quads)
				{
					...
					if (find(vs.begin(), vs.end(), v0) != vs.end())
					{
						...
						for (uint32_t j = 0; j < vs.size(); j++)
							h.vs.push_back(vs[(id + j) % vs.size()]);
						...
						for (uint32_t j = 1; j < h.vs.size(); j++)
							for (auto vid2 : vs3)
							{
			...
			else if (quadN == 6)
			{
				...
				for (auto vid : hvs)
					if (find(h.vs.begin(), h.vs.end(), vid) != h.vs.end())
						...
						vs4.push_back(vid);
				for (auto vid : hybrid_standard.Fs[quads[0]].vs)
					for (auto vid2 : vs4)
					{
						vector<uint32_t> es0 = hybrid_standard.Vs[vid].neighbor_es, es1 = hybrid_standard.Vs[vid2].neighbor_es, sharedes;
						sort(es0.begin(), es0.end());
...
void grid_hex_meshing_bijective::connectivity_modification(Mesh &hybrid_standard, vector<Element_Type> &h_type, vector<NV_type> &nv_tag, Mesh &pure_hex_mesh)
{
	...
	for (uint32_t i = 0; i < f_layers.size(); i++)
		for (auto fid : f_layers[i])
			for (auto vid : hybrid_standard.Fs[fid].vs)
			{
				;
			}
		...
		while (true)
		{
			...
			for (auto fid : a_group)
				if (!tq_tag[fid])
				{
					...
					if (e_indices.size() == 2)
					{
						if ((e_indices[0] + 1) % 4 == e_indices[1] || (e_indices[1] + 1) % 4 == e_indices[0])
						{
							s_fid = fid;
							break;
						}
					}
					else if (e_indices.size() > 2)
					{
						s_fid = fid;
						break;
					}
			...
			if (s_fid == -1)
			{
				for (auto fid : a_group)
					if (!tq_tag[fid])
					{
						...
						{
							int eid = hybrid_standard.Fs[fid].es[i];
							for (auto nfid : hybrid_standard.Es[eid].neighbor_fs)
								if (fid != nfid && tip_quads[nfid])
								{
									vector<int> pyramids0 = bcells[fid].pyramid_ids, pyramids1 = bcells[nfid].pyramid_ids, sharedpyramids;
									sort(pyramids0.begin(), pyramids0.end());
									sort(pyramids1.begin(), pyramids1.end());
						...
						for (uint32_t i = 0; i < 4; i++)
						{
							...
							for (auto nfid : hybrid_standard.Es[eid].neighbor_fs)
								if (fid != nfid && tip_quads[nfid])
								{
									...
									if (sharedpyramids.size() == 0)
									{
										e_indices_notshare.push_back(i);
										break;
									}
									else
									{
										e_indices_share.push_back(i);
										break;
									}
								}
						}
			...
			if (s_fid == -1)
			{ // more than 4 quads:open cylinder
				for (const auto &fid : a_group)
					if (!tq_tag[fid])
					{
						vector<int> e_indices_share, e_indices_notshare;
			...
			vector<int> pool, pool_;
			pool.push_back(s_fid);
			while (pool.size())
			{
				for (auto fid : pool)
					for (uint32_t i = 0; i < 4; i++)
					{
						int eid = hybrid_standard.Fs[fid].es[i];
						for (auto nfid : hybrid_standard.Es[eid].neighbor_fs)
							if (!tq_tag[nfid] && tip_quads[nfid] && q_tag[nfid] == Quad_Tag::Initial_State)
							{

								int whichindex = -1;
								for (uint32_t j = 0; j < 4; j++)
									if (eid == hybrid_standard.Fs[nfid].es[j])
									{
										whichindex = j;
										break;
									}
								int whichindex_o = -1;
								for (uint32_t j = 0; j < 4; j++)
									if (eid == hybrid_standard.Fs[fid].es[j])
									{
										whichindex_o = j;
										break;
									}
								...
								{
									if (sharedpyramids.size() == 0)
										q_tag[nfid] = Quad_Tag::Entire_Split;
									else if (sharedpyramids.size() == 1)
										q_tag[nfid] = Quad_Tag::No_Split;
									else if (sharedpyramids.size() == 2)
									{
										if (whichindex == 0 || whichindex == 2)
											q_tag[nfid] = Quad_Tag::One_Split_x;
										else if (whichindex == 1 || whichindex == 3)
											q_tag[nfid] = Quad_Tag::One_Split_y;
									}
								}
								else if (q_tag[fid] == Quad_Tag::One_Split_x)
								{
									...
									{
										if (whichindex_o == 0 || whichindex_o == 2)
											q_tag[nfid] = Quad_Tag::Entire_Split;
										else if (whichindex_o == 1 || whichindex_o == 3)
											q_tag[nfid] = Quad_Tag::No_Split;
									}
								}
	...
	for (auto agroup : f_layers)
		for (auto fid : agroup)
			if (q_tag[fid] == Quad_Tag::Entire_Split)
				for (auto eid : hybrid_standard.Fs[fid].es)
				{
					edge_tag[eid] = true;
				}
	...
	{
		if (vertex_type[i] != Vertex_Tag::Pyramid_tip)
			continue;

		vector<int> es, fs; // tips
		for (auto neid : hybrid_standard.Vs[i].neighbor_es)
			if (edge_tag[neid])
				es.push_back(neid);

		for (auto nfid : hybrid_standard.Vs[i].neighbor_fs)
			if (q_tag[nfid] == Quad_Tag::Entire_Split)
			{
				Hybrid_V v;
				v.v.resize(3, 0);
				...
				for (uint32_t j = 0; j < 3; j++)
					v.v[j] = hybrid_standard.Vs[which_v].v[j] + w_iq * bc.direction[j];
		...
		for (auto eid : es)
		{
			...
			if (nfs.size() == 1)
			{
				...
				for (uint32_t j = 0; j < 4; j++)
					if (hybrid_standard.Fs[nfs[0]].es[j] == eid)
					{
						if (bc.top_qvs[j] == i)
						{
				...
				for (uint32_t j = 0; j < 3; j++)
					v.v[j] = hybrid_standard.Vs[which_v].v[j] + w_e1 * bc.direction[j];
				v2tuple_map[i].push_back(tip_splits.size());
			...
			else if (nfs.size() == 2)
			{
				...
				for (auto fid : nfs)
				{
					...
					for (uint32_t j = 0; j < 4; j++)
						if (hybrid_standard.Fs[fid].es[j] == eid)
						{
							if (bc.top_qvs[j] == i)
							{
								which_v = bc.base_bvs[j][1];
					...
					for (uint32_t j = 0; j < 3; j++)
						v.v[j] += hybrid_standard.Vs[which_v].v[j] + w_e2 * bc.direction[j];
				}
				for (uint32_t j = 0; j < 3; j++)
					v.v[j] /= 2;
				v2tuple_map[i].push_back(tip_splits.size());
	...
	for (auto a_tip : tip_splits)
	{
		...
		for (uint32_t i = 0; i < 4; i++)
		{
			if (get<0>(a_tip) == bc.top_qvs[i])
				which_corner = i;
			if (get<2>(a_tip) != -1 && get<2>(a_tip) == hybrid_standard.Fs[bc.quad_id].es[i])
				which_side = i;
		}
		if (get<2>(a_tip) == -1)
		{
			if (which_corner != -1)
				bc.split_ivs[which_corner] = get<1>(a_tip);
			else
			{
				cout << "BUG!!!!";
			}
		...
		else
		{
			if (which_side != -1)
			{
				if (which_side == which_corner)
					bc.split_mvs[which_side][0] = get<1>(a_tip);
				else if (which_side == (which_corner + 1) % 4)
					bc.split_mvs[which_side][1] = get<1>(a_tip);
			}
			else
			{
				cout << "BUG!!!!";
	...
	{
		if (q_tag[i] == Quad_Tag::Initial_State || q_tag[i] == Quad_Tag::No_Split)
			continue;
		...
		if (q_tag[i] == Quad_Tag::Entire_Split)
		{
			...
			for (auto p : bc.prism_ids)
				h_tag[p] = false;
			for (auto p : bc.pyramid_ids)
				h_tag[p] = false;
			// bottom 8
			for (uint32_t j = 0; j < 4; j++)
			{
		...
		else
		{
			...
			if (q_tag[i] == Quad_Tag::One_Split_x)
			{
				...
				e0 = hybrid_standard.Fs[i].es[0];
				e2 = hybrid_standard.Fs[i].es[2];
			...
			else if (q_tag[i] == Quad_Tag::One_Split_y)
			{
				...
				e0 = hybrid_standard.Fs[i].es[1];
				e2 = hybrid_standard.Fs[i].es[3];
				vs0[0].push_back(hybrid_standard.Fs[i].vs[1]);
			...
			for (auto vid : vs2[0])
				if (find(bc2.top_qvs.begin(), bc2.top_qvs.end(), vid) != bc2.top_qvs.end())
				{
	...
	for (uint32_t i = 0; i < edge_tag.size(); i++)
		if (edge_tag[i])
		{
			// cout << i << endl;

			vector<int> nfs_entire, nfs_all;
			for (auto nfid : hybrid_standard.Es[i].neighbor_fs)
				if (tip_quads[nfid])
				{
					nfs_all.push_back(nfid);

					if (q_tag[nfid] == Quad_Tag::Entire_Split)
						nfs_entire.push_back(nfid);
				}
			...
			if (nfs_entire.size() == 2)
			{
				...
				for (auto fid : hybrid_standard.Hs[pid].fs)
				{
					if (find(hybrid_standard.Fs[fid].es.begin(), hybrid_standard.Fs[fid].es.end(), i) != hybrid_standard.Fs[fid].es.end())
					{
						for (auto nhid : hybrid_standard.Fs[fid].neighbor_hs)
							if (h_type[nhid] == Element_Type::Hexahedral && bc.hex_id != nhid)
								hid = nhid;
					}
				}
				for (auto fid : hybrid_standard.Hs[hid].fs)
					for (auto nhid : hybrid_standard.Fs[fid].neighbor_hs)
						if (h_type[nhid] == Element_Type::PyramidCombine)
							pids.push_back(nhid);
				if (!pids.size())
				{
					for (auto fid : hybrid_standard.Hs[hid].fs)
						for (auto nhid : hybrid_standard.Fs[fid].neighbor_hs)
							if (h_type[nhid] == Element_Type::TetCombine)
								pids.push_back(nhid);
					if (!pids.size())
					{
					...
					for (auto tid : v2tuple_map[vs0[0]])
						if (vs0[0] == std::get<0>(tip_splits[tid]) && i == std::get<2>(tip_splits[tid]))
						{
							vs0.push_back(get<1>(tip_splits[tid]));
							break;
						}
					vs1.push_back(hybrid_standard.Es[i].vs[1]);
					for (auto tid : v2tuple_map[vs1[0]])
						if (vs1[0] == std::get<0>(tip_splits[tid]) && i == std::get<2>(tip_splits[tid]))
						{
							vs1.push_back(get<1>(tip_splits[tid]));
							break;
						}
					...
					for (auto vfid : hybrid_standard.Vs[vs0[0]].neighbor_fs)
					{
						if (find(hybrid_standard.Hs[hid].fs.begin(), hybrid_standard.Hs[hid].fs.end(), vfid) != hybrid_standard.Hs[hid].fs.end())
						{
							...
							if (find(fvs.begin(), fvs.end(), vs1[0]) != fvs.end())
								continue;
							for (uint32_t j = 0; j < 4; j++)
								if (fvs[j] == vs0[0])
									vs00.push_back(fvs[(j + 2) % 4]);
							hfid0 = vfid;
							if (vs00.size())
								break;
						}
					...
					for (auto vfid : hybrid_standard.Vs[vs1[0]].neighbor_fs)
					{
						...
						{
							vector<uint32_t> &fvs = hybrid_standard.Fs[vfid].vs;
							if (find(fvs.begin(), fvs.end(), vs0[0]) != fvs.end())
								continue;
							for (uint32_t j = 0; j < 4; j++)
								if (fvs[j] == vs1[0])
									vs11.push_back(fvs[(j + 2) % 4]);
							hfid1 = vfid;
							if (vs11.size())
								break;
						}
					...
					sort(es00.begin(), es00.end());
					sort(es11.begin(), es11.end());
					...
					for (auto tid : v2tuple_map[vs00[0]])
						if (vs00[0] == std::get<0>(tip_splits[tid]) && oeid == std::get<2>(tip_splits[tid]))
						{
							vs00.push_back(get<1>(tip_splits[tid]));
							break;
						}
					for (auto tid : v2tuple_map[vs11[0]])
						if (vs11[0] == std::get<0>(tip_splits[tid]) && oeid == std::get<2>(tip_splits[tid]))
						{
							vs11.push_back(get<1>(tip_splits[tid]));
							break;
						}

					if (vs00.size() != 2 || vs11.size() != 2)
						continue;
				...
				for (uint32_t j = 0; j < 4; j++)
				{
					...
					if (j == 0)
						...
						h.vs[j + 4] = hybrid_standard.Hs[pids[0]].vs[j + 3];
				}
				pure_hex_mesh.Hs.push_back(h);
				// p1
				h.id = pure_hex_mesh.Hs.size();
				for (uint32_t j = 0; j < 4; j++)
				{
					...
					if (j == 0)
						...
						h.vs[j + 4] = hybrid_standard.Hs[pids[1]].vs[j + 3];
				}
				pure_hex_mesh.Hs.push_back(h);
				// hex
				h.id = pure_hex_mesh.Hs.size();
			...
			else if (nfs_entire.size() == 1 && nfs_all.size() == 1)
			{
				...
				int hid = -1;
				vector<int> pids, fids;
				for (auto fid : hybrid_standard.Hs[pid].fs)
				{
					if (find(hybrid_standard.Fs[fid].es.begin(), hybrid_standard.Fs[fid].es.end(), i) != hybrid_standard.Fs[fid].es.end())
					{
						for (auto nhid : hybrid_standard.Fs[fid].neighbor_hs)
							if (h_type[nhid] == Element_Type::Hexahedral && bc0.hex_id != nhid)
								hid = nhid;
					}
				}
				if (hid == -1)
				...
				for (auto vfid : hybrid_standard.Vs[vs_layers[1][0]].neighbor_fs)
				{
					...
					{
						vector<uint32_t> &fvs = hybrid_standard.Fs[vfid].vs;
						if (find(fvs.begin(), fvs.end(), vs_layers[1][1]) != fvs.end())
							continue;
						for (uint32_t j = 0; j < 4; j++)
							if (fvs[j] == vs_layers[1][0])
								vs_layers[1][2] = fvs[(j + 2) % 4];
					}
				}
				for (auto vfid : hybrid_standard.Vs[vs_layers[1][1]].neighbor_fs)
				{
					...
					{
						vector<uint32_t> &fvs = hybrid_standard.Fs[vfid].vs;
						if (find(fvs.begin(), fvs.end(), vs_layers[1][0]) != fvs.end())
							continue;
						for (uint32_t j = 0; j < 4; j++)
							if (fvs[j] == vs_layers[1][1])
								vs_layers[1][3] = fvs[(j + 2) % 4];
					}
				}
				int eid1 = -1;
				...
				for (auto nfid : hybrid_standard.Es[eid1].neighbor_fs)
					if (tip_quads[nfid])
					{
						nfs_all_.push_back(nfid);
						if (q_tag[nfid] == Quad_Tag::Entire_Split || q_tag[nfid] == Quad_Tag::One_Split_x || q_tag[nfid] == Quad_Tag::One_Split_y || q_tag[nfid] == Quad_Tag::No_Split)
							nfs_entire_.push_back(nfid);
					}
				...
				{
					for (auto vid : vs0)
						if (vid != vs_layers[1][0] && vid != vs_layers[1][2] && vid != vs_layers[4][1])
							vs_layers[0][0] = vid;
					for (auto vid : vs1)
						if (vid != vs_layers[1][1] && vid != vs_layers[1][3] && vid != vs_layers[4][2])
							vs_layers[0][1] = vid;
				}
				else
				{
...
void grid_hex_meshing_bijective::clean_hex_mesh(Mesh &tmi, Mesh_Domain &md)
{
	...
	for (uint32_t j = 0; j < signed_dis.size(); j++)
		if (signed_dis[j] < 0)
		{
			md.H_flag[j] = true;
		}
...
void grid_hex_meshing_bijective::tagging_uneven_element(const Mesh &mi, vector<bool> &H_flag)
{
	...
	for (int i = 0; i < H_flag.size(); i++)
	{
		...
		if (H_flag[i])
		{
			...
			{
				if (mi.Fs[fid].neighbor_hs.size() == 1)
					bn++;
				else if (!H_flag[mi.Fs[fid].neighbor_hs[0]] || !H_flag[mi.Fs[fid].neighbor_hs[1]])
					bn++;
				else if (H_flag[mi.Fs[fid].neighbor_hs[0]] && H_flag[mi.Fs[fid].neighbor_hs[1]])
					in++;
			}
	...
	while (true)
	{
		...
		for (auto v : hmi_local.Vs)
		{
			...
			for (auto fid : v.neighbor_fs)
				if (hmi_local.Fs[fid].boundary)
					bfs.push_back(fid);
			f_tag_.resize(bfs.size(), false);
			vector<uint32_t> f_pool;
			f_pool.push_back(bfs[0]);
			while (true)
			{
				vector<uint32_t> f_pool_;
				for (auto fid : f_pool)
				{
					if (f_tag_[find(bfs.begin(), bfs.end(), fid) - bfs.begin()])
						continue;
					else
						f_tag_[find(bfs.begin(), bfs.end(), fid) - bfs.begin()] = true;
					for (auto eid : hmi_local.Fs[fid].es)
					{
						for (auto nfid : hmi_local.Es[eid].neighbor_fs)
							if (find(bfs.begin(), bfs.end(), nfid) != bfs.end())
							{
								if (!f_tag_[find(bfs.begin(), bfs.end(), nfid) - bfs.begin()])
									f_pool_.push_back(nfid);
							}
					}
				}
				f_pool.clear();
			...
			if (non_manifold)
			{
				for (uint32_t i = 0; i < f_tag_.size(); i++)
				{
					if (!f_tag_[i])
					{
						...
						for (auto vid : hmi_local.Hs[nhid].vs)
							v_tag[vid] = true;
					}
				}
				changed = true;
		...
		if (!changed)
		{
			...
			for (auto e : hmi_local.Es)
			{
				...
				if (bfs.size() != 2)
				{
					...
					H_flag[H_map_reverse[nhid]] = false;
					for (auto fid : hmi_local.Hs[nhid].fs)
						for (auto eid : hmi_local.Fs[fid].es)
							e_tag[eid] = true;
				}
			}
...
void grid_hex_meshing_bijective::drop_small_pieces(Mesh_Domain &md)
{
		...
		for (uint32_t i = 0; i < a_group.size(); i++)
		{
			...
			for (auto fid : hmi_local.Hs[hid].fs)
			{
				for (auto nhid : hmi_local.Fs[fid].neighbor_hs)
					if (nhid != hid && !h_flag_[nhid])
						a_group.push_back(nhid);
			}
		}
	...
	if (hgroups.size() > 1)
	{
		...
		for (const auto &hid : hgroups[which])
			md.H_flag[md.H_map_reverse[hid]] = true;
...
void grid_hex_meshing_bijective::scaffold(Mesh_Domain &md)
{
	...
	for (int i = 0; i < args.scaffold_layer; i++)
	{
		...
		for (uint32_t j = 0; j < S_flag.size(); j++)
			if (S_flag[j])
			{
...
void grid_hex_meshing_bijective::break_circles(Mesh_Feature &mf_temp, vector<bool> &Corner_tag, vector<vector<uint32_t>> &circle2curve_map)
{
	...
	for (uint32_t i = 0; i < mf.circles.size(); i++)
	{
		if (mf.circles[i])
		{
			...
			circle2curve_map[i].push_back(i);
	}
}
bool grid_hex_meshing_bijective::node_mapping(Mesh_Feature &mf_temp, Feature_Graph &Tfg, Mesh_Domain &md)
{
	...
	int num_corner = 0;
	for (auto vid : mf_temp.corners)
	{
		Ps.row(num_corner++) = mf.tri.V.col(vid).transpose();
	}
	...
	for (uint32_t i = 0; i < mf_temp.corners.size(); i++)
	{
		...
		for (uint32_t j = 0; j < quad_tree.mesh.Fs[qid].vs.size(); j++)
		{
			...
			if (!V_flag[vid_])
				continue; // already taken by other corners
	...
	for (uint32_t i = 0; i < Tfg.Cs.size(); i++)
	{
		...
		for (auto cid : mf_temp.corner_curves[i])
		{
			...
			if (curve[0] == mf_temp.corners[i])
				vs.push_back(curve[1]);
			else if (curve[curve.size() - 1] == mf_temp.corners[i])
				vs.push_back(curve[curve.size() - 2]);
		}
		...
		{
			vector<int> cur_set;
			for (uint32_t j = 0; j < Qdirs_tag.size(); j++)
				if (Qdirs_tag[j])
					cur_set.push_back(j);
		...
		for (uint32_t j = 0; j < best_set.size(); j++)
			qc.ring_vs_tag[best_set[j]] = lids[j];
	}
	// corner deep mismatch check
	fill(V_flag.begin(), V_flag.end(), true);
	for (const auto &c : Qfg.Cs)
	{
		for (const auto &vid : c.vs)
			V_flag[vid] = false;
		for (const auto &vid : c.ring_vs)
			V_flag[vid] = false;
	}
	for (const auto &c : Qfg.Cs)
	{
...
bool grid_hex_meshing_bijective::curve_mapping(Feature_Graph &Tfg, Mesh_Domain &md)
{
	...
	for (auto &c : Qfg.Cs)
	{
		for (auto vid : c.vs)
			V_flag[vid] = false;
		for (uint32_t i = 0; i < c.ring_vs_tag.size(); i++)
			if (c.ring_vs_tag[i] != -1)
				V_flag[c.ring_vs[i]] = false;
	}
	...
	{
		for (auto &nv : v.neighbor_vs)
			adjacency_list[v.id].push_back(neighbor(nv, 1));
	}
	...
	for (uint32_t i = 0; i < Tfg.Ls.size(); i++)
	{
		...
		if (curve[0] == Tfg.Cs[end_cid].vs[0])
			swap(start_cid, end_cid);
		...
		if (start2_vid == Qfg.Cs[end_cid].vs[0] || Qfg.Cs[start_cid].vs[0] == end2_vid)
		{
			...
			for (auto vid : Qfg.Ls[i].vs)
				adjacency_list[vid].clear();
			continue;
		}
		...
		for (auto nv : quad_tree.mesh.Vs[start2_vid].neighbor_vs)
			adjacency_list[start2_vid].push_back(neighbor(nv, 1));
		for (auto nv : quad_tree.mesh.Vs[end2_vid].neighbor_vs)
			adjacency_list[end2_vid].push_back(neighbor(nv, 1));
	...
	Mesh_Feature &mf_temp, vector<bool> &Corner_tag, vector<vector<uint32_t>> &circle2curve_map, Mesh_Domain &md)
{
	...
	// compute Qfg
	for (uint32_t i = 0; i < Qfg.Cs.size(); i++)
		if (Corner_tag[i])
			mf_quad.corners.push_back(Qfg.Cs[i].vs[0]);
	...
	{
		if (!mf_quad.broken_curves[i])
			continue;

		for (auto vid : mf_quad.curve_vs[i])
			V_flag[QV_map_reverse[vid]] = true;
	...
	fc.V_types.resize(md.mesh_subA.Vs.size());
	fill(fc.V_types.begin(), fc.V_types.end(), Feature_V_Type::INTERIOR);
	for (auto c : Qfg.Cs)
		fc.V_types[QV_map_reverse[c.vs[0]]] = Feature_V_Type::CORNER;
	for (auto l : Qfg.Ls)
		for (auto vid : l.vs)
			if (fc.V_types[vid] != Feature_V_Type::INTERIOR && fc.V_types[vid] != Feature_V_Type::CORNER)
			{
				auto g_id = md.V_map_reverse[QV_map_reverse[vid]];
				tb_subdivided_cells.push_back(g_id);
	...
	// update mf_quad
	for (auto &vid : mf_quad.corners)
		vid = md.V_map_reverse[QV_map_reverse[vid]];
	for (auto &vs : mf_quad.curve_vs)
		for (auto &vid : vs)
			vid = md.V_map_reverse[QV_map_reverse[vid]];
...
bool grid_hex_meshing_bijective::patch_mapping(Mesh_Domain &md)
{
	...
	for (auto lid : tlines)
		if (tl_flag[lid])
			tfg_temp2.Ls.push_back(fg.Ls[lid]);
		else
		{
...
void grid_hex_meshing_bijective::patch_trees(Mesh_Domain &md)
{
	...
	for (uint32_t j = 0; j < graph_matches.size(); j++)
	{
		...
		for (auto rid : R)
			...
			{
				T_tag[tid] = true;
				for (auto vid : mf.tri.Fs[tid].vs)
					V_tag[vid] = true;
			}
		int vn = 0;
		vector<int> v_map(mf.tri.Vs.size(), -1);
		for (uint32_t i = 0; i < V_tag.size(); i++)
			if (V_tag[i])
				v_map[i] = vn++;
		Mesh tri_mesh;
		...
		for (uint32_t i = 0; i < V_tag.size(); i++)
			if (V_tag[i])
				tri_mesh.V.col(vn++) = mf.tri.V.col(i);
		for (uint32_t i = 0; i < T_tag.size(); i++)
			if (T_tag[i])
			{
				tri_mesh.Fs.push_back(mf.tri.Fs[i]);
				for (auto &vid : tri_mesh.Fs[tri_mesh.Fs.size() - 1].vs)
					vid = v_map[vid];
			}
...
void grid_hex_meshing_bijective::padding_connectivity(Mesh_Domain &md)
{
	...
	{
		Hybrid_V v_;
		v_.id = M_temp.Vs.size();
		v_.v.resize(3, 0);
		for (uint32_t i = 0; i < 3; i++)
			v_.v[i] = md.mesh_entire.V(i, v.id);
		M_temp.Vs.push_back(v_);
	}
	for (const auto &h : md.mesh_entire.Hs)
	{
		Hybrid h_;
		h_.id = M_temp.Hs.size();
		h_.vs = h.vs;
...
bool grid_hex_meshing_bijective::padding_local_untangle_geometry(Mesh_Domain &md)
{
	...
	int nI = 0;
	for (uint32_t i = 0; i < mq.H_Js.size(); i++)
		if (mq.H_Js[i] < Jacobian_Bound)
		{
			H_flag[i] = true;
			nI++;
		}
...
void grid_hex_meshing_bijective::local_padding_connectivity_robust(Mesh_Domain &md)
{
	...
	for (const auto &l : Qfg.Ls)
		for (const auto &eid : l.es)
			E_flag[eid] = l.id;

	if (Dirty_Vertices.size())
	{
		vector<bool> DV_flag(mf_quad.tri.Vs.size(), false);
		for (const auto &vid : Dirty_Vertices)
			DV_flag[md.QV_map[md.V_map[vid]]] = true;
		for (const auto &l : Qfg.Ls)
			for (const auto &eid : l.es)
			{
			...
			if (E_flag[f.es[i]] != -1)
			{
				...
				if (li_1 != -1 || li1 != -1)
				{
					if (li_1 != -1 && li != li_1)
					{
						...
						vector<Vector3d> ns;
						vector<int> vs;
						for (auto eid : mf.tri.Vs[vid].neighbor_es)
							if (tE_flag[eid] == li || tE_flag[eid] == li_1)
							{
								uint32_t v0 = mf.tri.Es[eid].vs[0];
								vs.push_back(v0);
								uint32_t v1 = mf.tri.Es[eid].vs[1];
								vs.push_back(v1);
					...
					if (li1 != -1 && li != li_1)
					{
						...
						vector<int> vs;
						for (auto eid : mf.tri.Vs[vid].neighbor_es)
							if (tE_flag[eid] == li || tE_flag[eid] == li1)
							{
								uint32_t v0 = mf.tri.Es[eid].vs[0];
								vs.push_back(v0);
								uint32_t v1 = mf.tri.Es[eid].vs[1];
	...
	for (auto qid : qs)
	{
		...
		for (uint32_t i = 0; i < 1; i++)
		{
			...
			{
				bool have_v = false;
				for (auto vid : m.Fs[qfid].vs)
					if (!V_tag[vid])
					{
						for (auto nfid : m.Vs[vid].neighbor_fs)
							if (m.Fs[nfid].on_medial_surface)
							{
								F_tag[nfid] = true;
								pool_.push_back(nfid);
							}
				...
				// if (!have_v)
				{
					for (auto eid : m.Fs[qfid].es)
						if (!E_tag[eid])
						{
							for (auto nfid : m.Es[eid].neighbor_fs)
								if (m.Fs[nfid].on_medial_surface)
								{
									F_tag[nfid] = true;
									pool_.push_back(nfid);
	...
	{
		bool foundone = false;
		for (; search_ahead < F_tag.size(); search_ahead++)
			if (F_tag[search_ahead] && m.Fs[search_ahead].on_medial_surface)
			{
		...
		while (region_.size())
		{
			vector<uint32_t> f_pool;
			for (auto f : region_)
				for (auto eid : m.Fs[f].es)
				{
					if (E_tag[eid])
						continue;
					for (auto nfid : m.Es[eid].neighbor_fs)
						if (F_tag[nfid] && m.Fs[nfid].on_medial_surface)
						{
																  ...
																  vector<bool> &H_tag, vector<bool> &F_tag, vector<int> &local_fs, vector<int> &local_hs)
{
	...
	// fs
	for (auto &hid : local_hs)
		for (auto &fid : md.mesh_entire.Hs[hid].fs)
			if (F_tag[fid])
				F_tag[fid] = false;
			else
				F_tag[fid] = true;
	...
	{
		vector<int> all_nhs;
		for (const auto &hid : md.mesh_entire.Vs[vid].neighbor_hs)
			if (H_tag[hid])
				all_nhs.push_back(hid);
		...
		while (true)
		{
			...
			while (true)
			{
				...
				{
					for (const auto &fid : md.mesh_entire.Hs[hid].fs)
						for (const auto &nhid : md.mesh_entire.Fs[fid].neighbor_hs)
																  ...
																  vector<bool> &H_tag, vector<int> &local_fs)
{
	...
	for (auto &lv : local_vs)
		v2lv_map[lv.hvid].push_back(lv.id);
	...
	for (auto &lv : local_vs)
	{
		...
		for (const auto &nhid : lv.nhs)
			vs_all.insert(vs_all.end(), md.mesh_entire.Hs[nhid].vs.begin(), md.mesh_entire.Hs[nhid].vs.end());
		sort(vs_all.begin(), vs_all.end());
		vs_all.erase(unique(vs_all.begin(), vs_all.end()), vs_all.end());

		Vector3d n;
		n.setZero();
		for (const auto &vid : vs_all)
			n += md.mesh_entire.V.col(vid) - md.mesh_entire.V.col(lv.hvid);
		n.normalize();
		for (uint32_t j = 0; j < 3; j++)
			v.v[j] = md.mesh_entire.V.col(lv.hvid)[j] + elen * Ratio_grow * n[j];
...
void grid_hex_meshing_bijective::feature_qmesh_udpate(Mesh_Domain &md)
{
	...
	for (auto &cid : mf_quad.corners)
		cid = QV_map[md.V_map[cid]];
	for (auto &vs : mf_quad.curve_vs)
		for (auto &vid : vs)
			vid = QV_map[md.V_map[vid]];
	for (uint32_t i = 0; i < mf_quad.curve_es.size(); i++)
		VSES(mf_quad.tri, mf_quad.curve_vs[i], mf_quad.curve_es[i], mf_quad.circles[i]);
	// cout << "update Qfg" << endl;
	build_feature_graph(mf_quad, Qfg);
	vector<bool> V_flag(md.mesh_entire.Vs.size(), true);
	for (const auto &v : hmi.Vs)
		if (!v.boundary)
			V_flag[md.V_map_reverse[v.id]] = false;
	for (const auto &vs : mf_quad.curve_vs)
		for (const auto &vid : vs)
			V_flag[md.V_map_reverse[QV_map_reverse[vid]]] = false;
	...
	for (auto &cid : mf_quad.corners)
		cid = md.V_map_reverse[QV_map_reverse[cid]];
	for (auto &vs : mf_quad.curve_vs)
		for (auto &vid : vs)
			vid = md.V_map_reverse[QV_map_reverse[vid]];
	for (uint32_t i = 0; i < mf_quad.curve_es.size(); i++)
...
void grid_hex_meshing_bijective::dirty_region_identification(Mesh_Domain &md, std::vector<int> &Dirty_Vs)
{
	...
	for (const auto &tc : fg.Cs)
	{
		...
		for (const auto &l : Qfg.Ls)
			for (const auto &vid : l.vs)
				C_flag[md.V_map_reverse[QV_map_reverse[vid]]] = l.id;
		...
		if (Has_Tiny)
		{
			...
			{
				if (tc.ring_vs_tag[i] >= 0)
					for (const auto &vid : Qfg.Ls[tc.ring_vs_tag[i]].vs)
						C_flag[md.V_map_reverse[QV_map_reverse[vid]]] = -1;
			}
				...
				for (int k = 0; k < lregion_vs.size(); k++)
				{
					...
					for (const auto &fid : nfs)
					{
						...
						for (const auto &vid : nvs)
							if (C_flag[md.V_map_reverse[QV_map_reverse[vid]]] != -1)
								continue;
							else
	...
	Dirty_Vs.clear();
	for (int i = 0; i < V_flag.size(); i++)
		if (V_flag[i])
			Dirty_Vs.push_back(i);
}
void grid_hex_meshing_bijective::projection_smooth(const Mesh &hmi, Feature_Constraints &fc)
{
	...
	for (int i = 0; i < fc.V_types.size(); i++)
		if (fc.V_types[i] == Feature_V_Type::REGULAR)
		{
			fc.ids_T(num_regulars_) = i;
			Ps.row(num_regulars_++) = hmi.V.col(i).transpose();
		}

	for (int i = 0; i < fc.ids_T.size(); i++)
	{
		Ps.row(i) = hmi.V.col(fc.ids_T[i]).transpose();
	}
...
void grid_hex_meshing_bijective::tet_assembling(const Mesh &m, const vector<Hybrid> &Hs_copies, const vector<bool> &H_flag, const vector<bool> &Huntangle_flag, bool local)
{
	...
	int count = 0;
	for (auto &h : hs)
		if ((local && Huntangle_flag[h.id]) || !local)
		{
			for (uint32_t i = 0; i < 8; i++)
			{
				for (uint32_t j = 0; j < 4; j++)
...
void grid_hex_meshing_bijective::local_region(const Mesh &m, const vector<bool> &HQ_flag, vector<bool> &H_flag, int &hn, VectorXi &b, MatrixXd &bc)
{
	...
	for (int i = 0; i < region_Size_untangle; i++)
	{
		...
		for (uint32_t j = 0; j < H_flag.size(); j++)
			if (H_flag[j])
			{
				for (const auto &vid : m.Hs[j].vs)
					for (const auto &nhid : m.Vs[vid].neighbor_hs)
						H_flag_[nhid] = true;
			}
		H_flag = H_flag_;
	}
	...
	hn = 0;
	for (const auto &g : H_flag)
		if (g)
			hn++;

	vector<bool> F_flag(m.Fs.size(), false);
	for (const auto f : m.Fs)
		if (!f.boundary)
		{
	...
	for (const auto &f : m.Fs)
		if (F_flag[f.id])
			for (const auto &vid : f.vs)
				if (!V_flag[vid])
				{
					V_flag[vid] = true;
					vi.push_back(vid);
					vs.push_back(m.V.col(vid).transpose());
				}
	b.resize(vi.size());
	bc.resize(vi.size(), 3);
...
bool grid_hex_meshing_bijective::stop_criterior_satisfied(Mesh_Domain &md, const int iter_after_untangle, const Mesh &tm0, Mesh &tm1, bool &max_dis_satisified, bool &ave_dis_satisfied)
{
	...
	auto &hmi = md.mesh_entire;
	for (int i = 0; i < md.TV_map_reverse.size(); i++)
		tm1.V.col(i) = hmi.V.col(md.V_map_reverse[md.TV_map_reverse[i]]);
	...
	if (iter_after_untangle >= start_ave_hausdorff_count_Iter)
	{
		...
		if (ratio_max < 0.8 * HR)
			max_dis_satisified = true;
		else
			max_dis_satisified = false;
		if (ratio_ave < 0.8 * STOP_AVE_HAUSDORFF_THRESHOLD)
			ave_dis_satisfied = true;
		else
			ave_dis_satisfied = false;
		if (ratio_ave < STOP_AVE_HAUSDORFF_THRESHOLD && ratio_max < HR)
			return true;
		if (ave_dis_satisfied)
			; // return true;
	}
	return false;
}
